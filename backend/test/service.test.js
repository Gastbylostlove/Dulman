...
import { validateLoginInput, validateRequestBody } from "../src/request-validation.js";
...
test("signup, login, chat flow, and reset are wired to the documented contracts", async () => {
  ...
  const senderView = await service.accessMedia(login.access_token, { message_id: media.message_id });
  assert.equal(senderView.view_count, 0);
...
test("request boundary validation rejects malformed bodies", () => {
